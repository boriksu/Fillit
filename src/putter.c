...
/*   Updated: 2019/06/10 16:44:56 by dholiday         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_b(t_etris *new_elem, char **setka, int size, char *str)
{
	t_kvadrat	b;
	int			past;

	if (new_elem == NULL)
		return (ft_search_r(setka, size));
	b.i = -1;
	while (++b.i < size - new_elem->high + 1)
	{
		b.j = new_elem->antiwide - 1;
		while ((++b.j < (size - new_elem->wide + 1)))
			if (setka[b.i][b.j] == '.' &&
